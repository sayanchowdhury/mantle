...
var (
	...
	awsPartitionsFedora = []awsPartitionSpec{
		awsPartitionSpec{
			Name:         "AWS",
			Profile:      "default",
			Bucket:       "coreos-prod-ami-import-us-west-2",
	...
	specs = map[string]channelSpec{
		...
		"fedora": channelSpec{
			BaseURL: "https://kojipkgs.fedoraproject.org/compose/updates/",
			Boards:  []string{""},
			AWS: awsSpec{
				BaseName:        "Fedora",
				BaseDescription: "Fedora AMI",
				Prefix:          "fedora_production_ami_",
				Image:           "fedora_production_ami_image.vmdk.bz2",
				Partitions:      awsPartitionsFedora,
			},
		},
	}
)
